...
Erweiterte Version mit formalem Entscheidungsautomaten, 5-Bit-Kodierung
und statistischer Analyse empirischer Abweichungen

Neue Funktionen:
- 5-Bit-Kodierung der Terminalzeichen
- Deterministischer endlicher Automat zur Wohlgeformtheitsprüfung
- Statistische Analyse von Übergangswahrscheinlichkeiten (Terminal- und Phasenebene)
- Erkennung von Schleifen und Wiederholungen
- Dokumentation fehlender Elemente (Begrüßung, Verabschiedung)
- Explizite Trennung von struktureller Validierung und statistischer Analyse
"""
    ...
    def validate_chain(self, coded_chain):
        ...
        for i, code in enumerate(coded_chain):
            ...
            protocol.append({
                ...
                'speaker': TerminalCoding.get_speaker(code),
    ...
    def get_history_string(self):
        ...
        for i, (state, code, explanation) in enumerate(self.history):
            if i == 0:
                ...
                phase = TerminalCoding.get_phase_name(code) if code else "-"
...
# ============================================================================
# STATISTISCHE ERWEITERUNG
# ============================================================================

class StatisticalExtension:
    """
    Statistische Erweiterung des formalen Automaten.
    Trennt strikt zwischen struktureller Validierung und statistischer Analyse.
    """
    ...
    def analyze_coded_chain(self, coded_chain, transcript_id):
        ...
        ohne die strukturelle Validierung zu beeinflussen.
        """
        # 1. Strukturelle Validierung (unabhängig)
        valid, state, protocol = self.automaton.validate_chain(coded_chain)
        
        # 2. Statistische Analyse (nachgelagert)
        self._count_transitions(coded_chain)
        self._count_phases(coded_chain)
        self._detect_loops(coded_chain, transcript_id)
        self._check_missing_elements(coded_chain)
    ...
    def _count_transitions(self, coded_chain):
        """Zählt Übergänge zwischen Terminalzeichen"""
        for i in range(len(coded_chain) - 1):
            curr = coded_chain[i]
            next_sym = coded_chain[i + 1]
            self.terminal_transitions[curr][next_sym] += 1
            self.terminal_counts[curr] += 1
    
    def _count_phases(self, coded_chain):
        """Zählt Übergänge zwischen Phasen (Bits 2-3)"""
        phases = [code[1:3] for code in coded_chain]
    ...
    def _detect_loops(self, coded_chain, transcript_id):
        """Erkennt Schleifen in der Sequenz"""
        # Einfache Schleifenerkennung: sich wiederholende Muster
        for length in range(2, min(5, len(coded_chain) // 2 + 1)):
            for i in range(len(coded_chain) - 2 * length + 1):
                pattern = coded_chain[i:i+length]
                if coded_chain[i+length:i+2*length] == pattern:
                    # Prüfe, ob dieses Muster bereits erfasst wurde
    ...
    def _check_missing_elements(self, coded_chain):
        """Prüft auf fehlende Elemente"""
        if not coded_chain:
            return
        
        # Fehlende Begrüßung?
        first = coded_chain[0]
        if first not in ['00000', '10000']:  # weder KBG noch VBG
            self.missing_elements['greeting'] += 1
        
        # Fehlende Verabschiedung?
        last = coded_chain[-1]
        if last not in ['01100', '11100']:  # weder KAV noch VAV
        ...
        for i in range(len(phases) - 1):
            curr = phases[i]
            next_phase = phases[i + 1]
            # Prüfe auf Phasenrücksprünge (z.B. A → B)
    ...
    def get_terminal_probabilities(self):
        """Berechnet Übergangswahrscheinlichkeiten auf Terminalebene"""
        probs = {}
    ...
    def get_phase_probabilities(self):
        """Berechnet Übergangswahrscheinlichkeiten auf Phasenebene"""
        probs = {}
    ...
    def print_report(self):
        ...
        lines.append(f"   Gültige Ketten: {valid_count}/{total} ({valid_count/total*100:.1f}%)")
        
        for result in self.transcript_results:
            status = "✓" if result['valid'] else "✗"
            lines.append(f"   Transkript {result['transcript_id']}: {status} "
                        f"(Länge: {result['length']}, Endzustand: {result['state']})")
        ...
        # 3. Schleifen
        lines.append("\n3. ERKANNTE SCHLEIFEN:")
        loops = self.get_loop_statistics()
        if loops:
            for i, loop in enumerate(loops[:5]):  # Top 5 anzeigen
        ...
        # 5. Terminal-Übergangswahrscheinlichkeiten (Top 10)
        lines.append("\n5. TERMINAL-ÜBERGANGSWAHRSCHEINLICHKEITEN (Top 10):")
    ...
    def export_json(self, filename="statistik_export.json"):
        ...
        stats['loops'] = [
            ...
            for l in stats['loops']
        ]
        stats['missing_elements'] = dict(stats['missing_elements'])
...
# ============================================================================
# ARS 3.0 - GRAMMATIK MIT NONTERMINALEN (gekürzt, da nicht im Fokus)
# ============================================================================
...
class ARSGUI:
    ...
    def create_statistics_tab(self):
        """Neuer Tab für statistische Analyse"""
...
- Statistischer Analyse empirischer Abweichungen
- Erkennung von Schleifen und Wiederholungen
- Expliziter Trennung von Struktur und Statistik
