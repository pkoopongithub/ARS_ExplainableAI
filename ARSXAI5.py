...
if HMM_AVAILABLE:
    class ARSHiddenMarkovModel:
        """
        Hidden-Markov-Modell für ARS 5.0
        ...
        def prepare_data(self, coded_chains):
            """
            Bereitet kodierte Ketten für HMM vor.
            ...
            self.symbol_to_idx = {code: i for i, code in enumerate(sorted(all_codes))}
            self.idx_to_symbol = {i: code for code, i in self.symbol_to_idx.items()}
            self.n_features = len(all_codes)
            
            # Daten in Sequenzen konvertieren (nur gültige Codes)
            X = []
            lengths = []
            ...
            for chain in coded_chains:
                seq = []
                for code in chain:
                    if code in self.symbol_to_idx:
                        seq.append(self.symbol_to_idx[code])
                    # Unbekannte Codes werden ÜBERSPRUNGEN (sie sind strukturell falsch)
        ...
        def initialize_from_structure(self, coded_chains):
            ...
            aber die Wahrscheinlichkeiten werden empirisch ermittelt.
            """
        ...
        def fit(self, coded_chains, n_iter=100):
            ...
            print(f"Gesamtlänge: {len(X)} Beobachtungen (nur strukturell gültige Codes)")
        ...
        def decode(self, coded_chain):
            ...
            # Konvertiere nur gültige Codes
            X_list = []
            for code in coded_chain:
                if code in self.symbol_to_idx:
                    X_list.append(self.symbol_to_idx[code])
                else:
                    # Unbekannte Codes werden als -1 markiert (später behandelt)
                    X_list.append(-1)
            
            if not X_list:
                return None, None
            
            # Entferne -1 für die Dekodierung (strukturell ungültige Positionen)
            valid_indices = [i for i, val in enumerate(X_list) if val != -1]
            valid_X = [X_list[i] for i in valid_indices]
            ...
            try:
                ...
                full_states = [-1] * len(X_list)
                for idx, state in zip(valid_indices, states):
                    full_states[idx] = state
...
class ARSGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("ARS 5.0 - Algorithmic Recursive Sequence Analysis")
...
KBG, VBBd, KBBd, VBA, VAA, KAA, VAV, KAV"""
    ...
    def show_about(self):
        about = """ARS 5.0 - Algorithmic Recursive Sequence Analysis
