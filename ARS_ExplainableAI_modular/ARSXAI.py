...
# -*- coding: utf-8 -*-
"""
ARSXAI.py - Algorithmic Recursive Sequence Analysis with Explainable AI
========================================================================

VOLLSTÄNDIGE REFERENZIMPLEMENTIERUNG (linear, lesbar, dokumentiert)

Version: 10.0 (Kern-Implementierung)
...
ERWEITERUNGEN (separate Module):
- arsxai_ext_depth.py   : Depth-Bounded PCFG (Tiefenbeschränkung)
- arsxai_ext_mdl.py     : MDL-Optimierung (Kompression als Gütekriterium)
- arsxai_ext_prefixspan.py : PrefixSpan für große Korpora
- arsxai_ext_seminfo.py : SemInfo-Maximierung für semantische Namen
"""
...
def check_core_packages():
    """Prüft und installiert fehlende Kernpakete"""
    print("=" * 70)
    print("ARSXAI - KERNPAKETPRÜFUNG")
    print("=" * 70)
    ...
    if missing_packages:
        print("\nInstalliere fehlende Kernpakete...")
...
class XAIModel:
    ...
    def train(self, chains: List[List[str]]) -> bool:
        """Trainiert das Modell auf den Daten"""
        raise NotImplementedError
    
    def explain(self, data: Any, detail_level: str = 'normal') -> Dict:
...
class MethodologicalReflection:
    ...
    def log_interpretation(self, sequence: Tuple, new_nonterminal: str, rationale: str):
...
class GrammarInducer(XAIModel):
    ...
    def __init__(self):
        ...
        self.symbol_to_nonterminals: Dict[str, Set[str]] = defaultdict(set)
    ...
    def _find_best_repetition(self, chains: List[List[str]], min_length: int = 2, max_length: int = 5) -> Optional[Tuple]:
    ...
    def _generate_rationale(self, sequence: Tuple) -> str:
        ...
        percentage = (occurrences / total) * 100 if total > 0 else 0
    ...
    def _sequence_in_chain(self, sequence: Tuple, chain: List[str]) -> bool:
    ...
    def _compress_sequences(self, chains: List[List[str]], sequence: Tuple, new_nonterminal: str) -> List[List[str]]:
    ...
    def _all_chains_identical(self, chains: List[List[str]]) -> bool:
        if not chains:
            return False
        first = chains[0]
        return all(len(chain) == 1 and chain[0] == first[0] for chain in chains)
    
    def _find_top_level_nonterminal(self) -> Optional[str]:
    ...
    def _count_expansions(self, sequence: List[str], expansion_counts: Dict):
        i = 0
        ...
        if isinstance(data, list):
            ...
            for hist in self.compression_history:
                if tuple(hist['sequence']) == seq:
                    percentage = (hist['occurrences'] / len(self.chains)) * 100 if self.chains else 0
    ...
    def generate_chain(self, start_symbol: Optional[str] = None, max_depth: int = 20) -> List[str]:
        ...
        def expand(symbol: str, depth: int = 0) -> List[str]:
            if depth >= max_depth:
                return [str(symbol)]
...
class NaturalLanguageExplainer:
    ...
    def __init__(self, grammar_inducer: GrammarInducer):
        self.grammar = grammar_inducer
        self.pattern_names = self._generate_pattern_names()
    
    def _generate_pattern_names(self) -> Dict[str, str]:
    ...
    def explain_sequence(self, sequence: List[str]) -> str:
                ...
                for prod, prob in self.grammar.rules.get(nt, []):
                    if prod == current:
                        for hist in self.grammar.compression_history:
                            if hist['new_symbol'] == nt:
                                percentage = (hist['occurrences'] / len(self.grammar.chains)) * 100 if self.grammar.chains else 0
                                break
                        else:
                            percentage = 0
        ...
        if occurrences > 0:
            percentage = (occurrences / len(self.grammar.chains)) * 100 if self.grammar.chains else 0
            lines.append("")
            lines.append(f"📊 **Vorkommen**: {percentage:.0f}% der Ketten ({occurrences} von {len(self.grammar.chains)})")
        
        return "\n".join(lines)
    
    def _sequence_in_chain(self, sequence: List[str], chain: List[str]) -> bool:
        """Prüft, ob eine Sequenz in einer Kette vorkommt"""
        seq_len = len(sequence)
        for i in range(len(chain) - seq_len + 1):
...
# ============================================================================
# ARS 2.0 - BASIS-GRAMMATIK (für Vergleich)
# ============================================================================
...
class ARS20(XAIModel):
    ...
    def train(self, chains: List[List[str]], start_symbol: Optional[str] = None) -> bool:
        self.chains = chains
        all_terminals = set()
        for chain in chains:
            for symbol in chain:
    ...
    def _count_transitions(self, chains: List[List[str]]) -> Dict:
        transitions = {}
        for chain in chains:
    ...
    def _calculate_probabilities(self, transitions: Dict) -> Dict:
        probabilities = {}
    ...
    def generate_chain(self, start_symbol: Optional[str] = None, max_length: int = 20) -> List[str]:
        if not self.trained:
            return []
...
class ARSHiddenMarkovModel(XAIModel):
    ...
    def train(self, chains: List[List[str]], n_iter: int = 100) -> bool:
    ...
    def _prepare_data(self, chains: List[List[str]]) -> Tuple[np.ndarray, np.ndarray]:
        all_symbols = set()
        for chain in chains:
            for sym in chain:
                all_symbols.add(sym)
    ...
    def decode_chain(self, chain: List[str]) -> Tuple[Optional[np.ndarray], Optional[float]]:
    ...
    def generate_chain(self, max_length: int = 20) -> List[str]:
        if not self.trained or self.model is None:
            return []
...
class ARSCRFModel(XAIModel):
    ...
    def train(self, chains: List[List[str]], max_iterations: int = 100) -> bool:
    ...
    def _prepare_data(self, sequences: List[List[str]]) -> Tuple[List, List]:
        X = []
        y = []
    ...
    def _extract_features(self, sequence: List[str], i: int) -> Dict:
        features = {
            'bias': 1.0,
            'symbol': sequence[i],
            'position': i,
    ...
    def get_feature_string(self, n: int = 20) -> str:
        if not hasattr(self.crf, 'state_features_'):
...
if MODULE_STATUS['networkx']:
    class ARSPetriNet(XAIModel):
        ...
        def __init__(self, name: str = "ARS_PetriNet"):
        ...
        def add_place(self, name: str, initial_tokens: int = 0, place_type: str = "normal"):
            self.places[name] = {
                'name': name,
                'type': place_type,
        ...
        def add_transition(self, name: str, transition_type: str = "speech_act", guard=None, subnet=None):
        ...
        def _calculate_confidence(self, chains: List[List[str]]) -> float:
            if not chains:
                return 0.0
...
class ChainGenerator(XAIModel):
    ...
    def set_source_model(self, model: XAIModel):
        self.source_model = model
    
    def generate(self, count: int = 5, max_length: int = 20) -> List[List[str]]:
...
class XAIModelManager:
    ...
    def __init__(self):
        self.models: Dict[str, XAIModel] = {}
        self.active_models: Set[str] = set()
    
    def register_model(self, name: str, model: XAIModel):
    ...
    def train_all(self, chains: List[List[str]]) -> Dict:
        results = {}
    ...
    def explain_all(self, data: Any, detail_level: str = 'normal') -> Dict:
        explanations = {}
        for name in self.active_models:
            if name in self.models:
                try:
...
class DataValidator:
    ...
    def validate_chains(self, chains: List[List[str]]) -> Tuple[List, List]:
        self.issues = []
        self.warnings = []
    ...
    def _group_similar_symbols(self, symbols: Set[str]) -> Dict:
...
class DerivationVisualizer:
    ...
    def __init__(self, root, plot_thread: PlotThread):
        self.root = root
        self.plot_thread = plot_thread
    
    def plot_grammar_hierarchy(self, grammar: GrammarInducer):
    ...
    def plot_pattern_frequency(self, grammar: GrammarInducer):
        ...
        for hist in grammar.compression_history[:10]:
            patterns.append(hist['new_symbol'])
            freq = (hist['occurrences'] / len(grammar.chains)) * 100 if grammar.chains else 0
...
class MultiFormatExporter:
    ...
    def to_json(self, data: Dict, filename: Optional[str] = None) -> str:
        if filename is None:
    ...
    def to_html(self, data: Dict, filename: Optional[str] = None) -> str:
        if filename is None:
        ...
        html.append("<html>")
        html.append("<head><title>ARSXAI Analysebericht</title>")
        html.append("<style>")
        ...
        html.append(f"<h1>ARSXAI Analysebericht</h1>")
    ...
    def to_latex(self, data: Dict, filename: Optional[str] = None) -> str:
        if filename is None:
        ...
        latex.append("\\title{ARSXAI Analyseergebnisse}")
...
# ============================================================================
# GUI - HAUPTFENSTER (Basisklasse)
# ============================================================================

class ARSXAIBaseGUI:
    """
    Basis-GUI für ARSXAI.
    Kann für Erweiterungen überschrieben werden.
    """
    
    def __init__(self, root):
        self.root = root
        self.root.title("ARSXAI - PCFG-basierte Musteranalyse mit XAI")
        ...
        self.grammar = GrammarInducer()
        self.explainer: Optional[NaturalLanguageExplainer] = None
    ...
    def _register_models(self):
        ...
        if MODULE_STATUS['networkx'] and 'ARSPetriNet' in dir():
    ...
    def get_actual_delimiter(self) -> str:
    ...
    def toggle_model(self, name: str):
    ...
    def show_export_dialog(self):
        ...
        for hist in self.grammar.compression_history:
            patterns.append({
                ...
                'frequency': (hist['occurrences'] / len(self.chains)) * 100 if self.chains else 0,
    ...
    def load_example(self):
        example = """# Beispieltranskripte mit C-Symbolen
...
CBG, BBG, CBBd, BBBd, CBA, BBA, BAA, CAA, BAB, CAB"""
    ...
    def show_about(self):
        about = """ARSXAI - Algorithmic Recursive Sequence Analysis with Explainable AI
...
• PrefixSpan (arsxai_ext_prefixspan.py)
• SemInfo (arsxai_ext_seminfo.py)

© 2024 - Explainable AI Research"""
        
        messagebox.showinfo("Über ARSXAI", about)
...
def main():
    """Hauptfunktion"""
    print("\n" + "=" * 70)
    print("ARSXAI - KERNIMPLEMENTIERUNG GESTARTET")
    print("=" * 70)
    print("Für Erweiterungen: importiere arsxai_ext_*.py")
    print("=" * 70 + "\n")
    
    root = tk.Tk()
    app = ARSXAIBaseGUI(root)
    root.mainloop()
