...
author:
- Paul Koop
date: Lehr- und Lernmaterial 2026
title: |
  **Zwischen Interpretation und Berechnung**\
  Didaktische Exploration computerlinguistischer Verfahren\
  mit augmentierten Transkripten von Verkaufsgesprächen
---

# Einleitung: Didaktische Ziele und methodologische Reflexion
...
# Die acht Transkripte: Rohdaten und Terminalzeichen
...
``` {caption="Transkript 1 - Rohdaten" basicstyle="\\ttfamily\\footnotesize"}
Kunde: Guten Tag!
...
``` {caption="Transkript 2 - Rohdaten" basicstyle="\\ttfamily\\footnotesize"}
...
``` {caption="Transkript 3 - Rohdaten" basicstyle="\\ttfamily\\footnotesize"}
...
``` {caption="Transkript 4 - Rohdaten" basicstyle="\\ttfamily\\footnotesize"}
...
``` {caption="Transkript 5 - Rohdaten" basicstyle="\\ttfamily\\footnotesize"}
...
``` {caption="Transkript 6 - Rohdaten" basicstyle="\\ttfamily\\footnotesize"}
...
``` {caption="Transkript 7 - Rohdaten" basicstyle="\\ttfamily\\footnotesize"}
...
``` {caption="Transkript 8 - Rohdaten" basicstyle="\\ttfamily\\footnotesize"}
...
## Die Terminalzeichenketten (ARS 3.0)
...
# Szenario C: Computerlinguistische Integration
...
``` {.python caption="Data Augmentation für Lehrzwecke" language="Python"}
...
## Speech Act Recognition mit Transformer-Modellen

Die automatische Erkennung der Sprechakte erfolgt mit einem feinabgestimmten BERT-Modell:

``` {.python caption="Speech Act Recognition mit BERT" language="Python"}
"""
...
``` {.python caption="Semantische Ähnlichkeit mit Word Embeddings" language="Python"}
"""
...
``` {.python caption="Topic Modeling mit BERTopic" language="Python"}
"""
...
## Integration der Komponenten in Szenario C

Die vollständige Integration aller Komponenten in Szenario C:

``` {.python caption="Szenario C - Vollständige Integration" language="Python"}
"""
...
``` {.python caption="CRF für sequenzielle Abhängigkeiten" language="Python"}
"""
...
## Transformer-Embeddings als Ergänzung

Transformer-Embeddings werden zusätzlich zu den kategorialen Terminalzeichen verwendet:

``` {.python caption="Transformer-Embeddings für Terminalzeichen" language="Python"}
"""
...
## Graph Neural Networks für die Nonterminal-Hierarchie

Die Nonterminal-Hierarchie wird als Graph Neural Network modelliert:

``` {.python caption="Graph Neural Network für Nonterminal-Hierarchie" language="Python"}
"""
...
``` {.python caption="Attention-Mechanismen für Sequenzmodellierung" language="Python"}
"""
...
## Integration der Komponenten in Szenario D

Die vollständige Integration aller Komponenten in Szenario D:

``` {.python caption="Szenario D - Vollständige hybride Integration" language="Python"}
"""
Szenario D: Hybride Modellierung
...
# Vergleich der Szenarien und methodologische Reflexion
...
## Didaktische Erkenntnisse aus Szenario C
...
## Didaktische Erkenntnisse aus Szenario D
...
## Fazit für die Lehrpraxis

Die didaktische Exploration der Szenarien C und D führt zu folgenden Schlussfolgerungen:

1.  **Szenario C eignet sich zur Demonstration der Funktionsweise** neuronaler Verfahren, sollte aber mit explizitem Hinweis auf die fehlende empirische Validität und die methodologischen Probleme eingesetzt werden.
